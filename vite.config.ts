...
function typescriptMimePlugin(): Plugin {
  const patchResponse = (req: { url?: string }, res: ServerResponse): void => {
    if (req.url && /^\/(src|node_modules)\/.+\.(ts|tsx|mts)(\?.*)?$/.test(req.url)) {
      ...
      // Restore the original method once the response is complete so the
      // patched method does not persist beyond this request.
      res.once('finish', () => {
  ...
  return {
    ...
    configureServer(server) {
      // Register WITHOUT returning a function so this runs as a *pre*
      // middleware — i.e. before Vite's internal transform/static
      // middlewares. The setHeader patch must already be installed when
      // Vite assigns the Content-Type, otherwise the OS-derived
      // "text/vnd.trolltech.linguist" type is sent before we can correct it.
